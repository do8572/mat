...
class gradient_descent:
    ...
    def minimize(self, f, df, ddf, x0, method="GD", num_steps=1000):
        ...
        x_star = None
        if method == "GD":
            x_star = self.GD(x0, gamma=0.001, num_steps=num_steps)
        elif method == "Polyak":
            x_star = self.PolyakGD(x0, gamma=0.001, mu=0.01, num_steps=num_steps)
        elif method == "Nesterov":
            x_star = self.NesterovGD(x0, gamma=0.001, mu=0.01, num_steps=num_steps)
        elif method == "AdaGrad":
            x_star = self.AdaGradGD(x0, gamma=0.001, num_steps=num_steps)
        elif method == "Newton":
    ...
    def PolyakGD(self, x0, gamma=None, mu=None, num_steps=1000):
        ...
        for _ in range(num_steps):
            x0 = xk
            xk = xk - gamma * self.df(xk) + mu * (xk - x0)
            if np.max(self.df(xk)) < self.tol:
                break
        return xk
...
class LinearRegression:
    ...
    def fit(self, N, opt):
        ...
        # TODO: function derivatives
        f = lambda v: 0.5 * np.sum((self.X.dot(v) - self.y)**2)
...
if __name__ == "__main__":
    ...
    df2 = lambda x: np.array([2*(x[0]-x[2])+8*(4*x[0]-2*x[1]+x[2])+1, \
                              4*(2*x[1] + x[2]) - 4 * (4*x[0] - 2*x[1]+x[2]), \
                             -2*(x[0] - x[2]) + 2*(2*x[0]+ x[2]) + (2*x[0]-2*x[1]+x[2]) 
                            ])
    ddf2 = lambda x: np.array([[34, -16, 6], [-16,16,0], [6,0,5]])
